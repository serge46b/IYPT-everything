...
# SETTINGS
# file name
FILE_NAME = "./46 ball/46 ball data.xlsx"
VIDEO_FILE_NAME = "./16 ball/16 ball rotated.avi"
OUT_LOG_FILE_NAME = "./46 ball rotation data.xlsx"
# timing
ST_FRAME_IDX = 0
END_FRAME_IDX = 20000
# Logging
...
for i in data.iloc:
    # ret, frame = cap.read()
    # if i["frame"] < st_f or i["frame"] > end_frame:
    #     continue
    obj_rel_x, obj_rel_y = i["object rotated x"], i["object rotated y"]
    # if revolution_x is None or revolution_y is None:
    #     revolution_x = obj_rel_x
    #     revolution_y = obj_rel_y
    #     continue
    box_center_x, box_center_y = (i["cluster center rotated x"], i["cluster center rotated y"])
    ...
    # )
    angle = find_angle_between_two_lines(
        (box_center_x, box_center_y+1),
        (box_center_x, box_center_y),
        (obj_rel_x, obj_rel_y)
    )
    # frame = cv2.circle(frame, (box_center_x, box_center_y+1), 2, (0, 255, 255), -1)
    # frame = cv2.circle(frame, (box_center_x, box_center_y), 2, (255, 255, 0), -1)
    # frame = cv2.circle(frame, (obj_rel_x, obj_rel_y), 2, (255, 0, 255), -1)
    # cv2.imshow("frame", frame)
    # print(angle)
    # cv2.waitKey(0)
    ...
    if abs(prev_angle - angle) > 300 and flag:
        ...
        revolution_y = obj_rel_y
    prev_angle = angle
    out_data = pd.concat([out_data, pd.DataFrame({"frame": i['frame'], "angle": angle - rev_n * 360}, index=[0])])

print("writing to xlsx file...")
with pd.ExcelWriter(OUT_LOG_FILE_NAME) as writer:
    out_data.to_excel(writer, sheet_name="data", index=False)
print("done")
