...
dict_main_aruco = aruco.Dictionary_get(aruco.DICT_4X4_1000)
parameters = aruco.DetectorParameters_create()
camera = cv2.VideoCapture(3, apiPreference=cv2.CAP_ANY, params=[
    cv2.CAP_PROP_FRAME_WIDTH, 1920,
    cv2.CAP_PROP_FRAME_HEIGHT, 1080])


ROOT = "./2023/Cushion Catapult/"
path = ROOT + "calibration images/"
count = 0
while camera.isOpened():
    ...
    corners, ids, rejectedImgPoints = aruco.detectMarkers(
        gray, dict_main_aruco, parameters=parameters)
    list_ids = np.ravel(ids)
    draw_frame = img.copy()
    if len(corners) > 0:
        for i in range(len(corners)):
            draw_frame = cv2.polylines(
                draw_frame, [corners[i].astype(int)], True, (0, 255, 0), 1)
            draw_frame = cv2.rectangle(draw_frame, corners[i][0][0].astype(int) - 3, corners[i][0][0].astype(int) + 3,
                                       (0, 0, 255), 1)
    cv2.imshow("tags", resize_with_aspect_ratio(
        draw_frame, height=DISPLAY_IMG_HEIGHT))
